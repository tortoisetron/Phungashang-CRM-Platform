...
export default function ProductCard({ product }) {
    ...
    return (
        <div className="group overflow-hidden relative border border-gray-200 hover:border-blue-300 shadow-sm hover:shadow-xl hover:shadow-blue-900/10 rounded-2xl bg-white flex flex-col h-full transition-all duration-300">
            ...
            {/* Image / Book wrapper */}
            <div className="relative aspect-[3/4] overflow-hidden bg-gradient-to-br from-blue-50 to-slate-100 p-6 flex items-center justify-center">
                {/* Book styling effect */}
                <div className="w-[85%] h-[90%] relative group-hover:-translate-y-2 group-hover:scale-105 transition-all duration-500 shadow-[10px_20px_30px_-10px_rgba(30,58,138,0.3)]">
                    ...
                    <div className="absolute inset-0 z-10 rounded-r-lg rounded-l-sm border-l-[8px] border-[#0f172a]/90 overflow-hidden transform perspective-1000 rotateY-[-5deg]">
                        <img 
                ...
                {/* Quick Add Overlay */}
                <div className="absolute inset-0 bg-white/70 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center p-6 z-30">
            ...
            <div className="p-6 flex flex-col flex-1 border-t border-gray-100">
                <div className="text-[10px] font-bold text-blue-600 mb-2.5 uppercase tracking-widest">
                    {product.category?.name || product.category || 'Nursing'}
                </div>
                <h3 className="font-bold text-[#111827] text-lg mb-3 line-clamp-2 leading-snug flex-1">
                     {product.name || product.title}
                </h3>
                <div className="flex items-center justify-between mt-auto">
                    <span className="text-2xl font-black text-[#111827] drop-shadow-sm">${product.price || '59.95'}</span>
                    <Link to={`/test-bank/${product.slug || product.id || ''}`} className="text-blue-600 font-bold text-sm tracking-wide hover:text-blue-800 transition-colors flex items-center gap-1 group/link">
                        View Details
